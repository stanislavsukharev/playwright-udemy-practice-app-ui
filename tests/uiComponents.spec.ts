...
test.describe('Form layots page', () => {
    ...
    test.only('radio buttons', async ({ page }) => {
        ...
        // Await the isChecked() method to properly retrieve the checked state
        const radioStatus = await usingTheGridEmailForm.getByRole('radio', { name: "Option 2" }).isChecked();
        await expect(usingTheGridEmailForm).toHaveScreenshot()
...
test('lists and dropdowns', async ({ page }) => {
    ...
    page.getByRole('list') //when the list gas UL tag
...
test('tooltips', async ({ page }) => {
    ...
    page.getByRole('tooltip') //if you have a role tooltip created in DOM
...
test('datepicker', async({page}) => {
    ...
    // await page.locator('.day-cell.ng-star-inserted').getByText('14').click() - this also works ('.2.classes')
